...
13. **Ошибки конечного сервиса рисует шлюз.** `proxy_intercept_errors on`
    стоит в блоке `location <prefix>/` сгенерированного конфига: ответ сервиса
    с кодом `4xx`/`5xx` теряет тело и заменяется страницей шлюза. Своя страница
    ошибки сервису не нужна, а JSON с кодом ошибки под его префиксом до
    `fetch()` не дойдёт. Причину передают заголовками `X-Error-Code` и
    `X-Error-Detail` — раздел 8.1
    [GATEWAY_SERVICE_INTEGRATION_API.md](integration/GATEWAY_SERVICE_INTEGRATION_API.md).
    Не перехватываются `<prefix>/static/` и `<prefix>/health`.
14. **Перехват не включать на маршрутах портала.** На server-блоке он выключен
    намеренно: страницы auth-service отдают `401` и `403` как контент (форма
    логина, `error.html`, `access-denied.html`). Перехват превращает их в
    редиректы, а `/access-denied` зацикливает — страница отдаёт `403`, шлюз
    шлёт `302` на неё же. Бюджет для браузера — **один редирект на ошибку**,
    поэтому цель редиректа перехватываться не должна никогда.
...
| браузер зациклился на редиректах | перехват включён на маршруте, который служит целью `error_page` (`/access-denied`, `/login`, `/menu`) |
