...
13. **Все страницы ошибок рисует шлюз.** `proxy_intercept_errors on` на
    server-блоке: ответ с кодом `4xx`/`5xx` теряет тело и заменяется страницей
    nginx — и у конечного сервиса, и у самого портала. Своя страница ошибки
    сервису не нужна, а JSON с кодом ошибки под его префиксом до `fetch()`
    не дойдёт. Причину передают заголовками `X-Error-Code` и `X-Error-Detail` —
    раздел 8.1
    [GATEWAY_SERVICE_INTEGRATION_API.md](integration/GATEWAY_SERVICE_INTEGRATION_API.md).
    Перехват снят там, где код — часть содержимого: `/login`,
    `/forgot-password`, `/reset-password`, `/api/`, ассеты, подзапросы
    `auth_request`, вебсокеты Dozzle, `<prefix>/static/` и `<prefix>/health`.
14. **`error_page` для перехватываемого кода ведёт на файл, а не на редирект.**
    Внутренний редирект на статику терминален; редирект возвращается клиенту,
    и если его цель отдаёт тот же код — получается цикл. Так было с
    `error_page 403 = 302 /access-denied`: страница отказа сама отдаёт `403`.
    Бюджет для браузера — **один редирект на ошибку**, и остался он только
    у `401` на `/login`.
...
- Управление ролями и правами отключено: `routes/role_management.go.disabled`.
  Маршруты `/roles`, `/permissions`, `/admin-menu` и `/service/` из nginx
  **убраны** — они проксировались в никуда и отдавали голый gin-овый
  `404 page not found`. Сейчас эти пути не совпадают ни с одним `location`
  и получают обычную страницу ошибки шлюза. Возвращать блоки в
  `gateway.inc` можно только вместе с работающими обработчиками.
...
| форма логина «проглотила» сообщение об ошибке | с маршрута сняли `proxy_intercept_errors off` — `401` формы стал редиректом |
| браузер зациклился на редиректах | `error_page` перехватываемого кода ведёт на `return 302`, а цель отдаёт тот же код |
