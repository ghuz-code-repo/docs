...
Интерфейс: `/menu`, `/profile`, `/settings`.
Админка: `/services`, `/users`,
...
Все страницы ошибок на портале рисует nginx. Собственных страниц отказа у
auth-service в потоке больше нет — их перехватывает и заменяет шлюз.

| Код | Поведение |
|---|---|
| `401` | `302 /login?redirect=$request_uri` — единственный редирект на весь шлюз |
| `403` на `/logs` | `302 /menu` |
| `403` на `/services/<key>/logs` | `302 /services/<key>` |
| все остальные 4xx и 5xx, включая `403` | стилизованная страница `html/errors/<код>.html` **на исходном URL**, без редиректа |

Кодов со страницей 39: `400`, `402`–`418`, `421`–`431`, `451`, `500`–`511`.
Каталог с текстами — `nginx/error-pages/codes.py`, там же генератор и тесты;
порядок правки — `nginx/error-pages/README.md`.

`401` страницы не получает намеренно. Это не отказ, а «представьтесь»:
пользователь ещё не назвался, и показывать ему тупик вместо формы входа
было бы вредно.

### Перехват ответов

`proxy_intercept_errors on` объявлен на уровне server-блока в `errors.inc`,
поэтому действует и на auth-service, и на автогенерируемые конфиги сервисов.
Без него `500` от сервиса уходил бы пользователю как есть — голой страницей
Flask или пустым телом.

**Тело ответа при перехвате выбрасывается.** Разбирать JSON nginx не умеет,
поэтому на странице остаётся то, что видно снаружи: код, `$upstream_status`,
`$upstream_addr`, адрес запроса, время и `$request_id`. Сервис, которому есть
что сказать, кладёт текст в заголовки ответа `X-Error-Code` и
`X-Error-Detail` — шлюз читает их как `$upstream_http_*` и показывает в блоке
подробностей.

#### Где перехват снят и почему

Не всякий не-2xx — авария. Там, где код является частью содержимого или
служебным сигналом, стоит `proxy_intercept_errors off`:

| Где | Почему |
|---|---|
| `/login` | отдаёт `401` вместе с формой и текстом «неверный пароль» (`routes/auth_handlers.go`) |
| `/forgot-password`, `/reset-password` | отдают `500` вместе со страницей результата |
| `^~ /api/`, `/check-user-exists` | JSON читает JS админки и `auth-connector`, а не человек |
| `/static/`, `/data/`, `/avatar/`, `/vite.svg`, `<prefix>/static/` | ассету нужен код ответа, а не страница на несколько килобайт |
| `= /verify`, `= /verify-admin`, `= /verify-logs-auth`, `= /verify-service-logs-auth` | подзапросы `auth_request` сами решают, что делать с `401` и `403` |
| `/logs`, `/services/<key>/logs` | вебсокеты Dozzle: подмена ответа рвёт соединение |
| `<prefix>/health` | опрашивают monitoring-service и docker |

Обратная сторона перехвата: **JSON-ответы сервиса с кодом 4xx и 5xx под его
префиксом до клиента не доходят** — `fetch()` получит HTML. Сервисам с
собственным API нужно либо проверять `content-type`, либо отдавать прикладные
ошибки кодом `200`. Ветка «JSON вместо страницы» заготовлена в `errors.inc`
и включается вместе с `map $http_accept $gw_wants_json` в `nginx.conf`.

### Мёртвые маршруты убраны

`/admin-menu`, `/roles`, `/permissions` и `/service/` больше не проксируются:
обработчиков с такими путями в auth-service нет (управление ролями лежит
в `routes/role_management.go.disabled`, импорт-экспорт переехал на
`/services/`). Проксирование отдавало голый `404 page not found` от gin —
единственное место на портале, где пользователь видел не оформленную
страницу.

Теперь эти пути не совпадают ни с одним `location`, `404` генерирует сам
nginx, и `error_page` подставляет обычную страницу ошибки. Обратно блоки
возвращают только вместе с работающими обработчиками.

### Сколько редиректов видит браузер

Не больше одного, и почти всегда — ноль:

| Ошибка | Что делает браузер |
|---|---|
| `401` от `auth_request` | один `302` на `/login?redirect=…`, дальше форма входа `200` |
| `403` на `/logs`, `/services/<key>/logs` | один `302` на рабочую страницу |
| всё остальное, включая `403` | **ноль**: страница отдаётся прямо на исходном URL с исходным кодом |

Так было не всегда. `error_page 403` вёл на `return 302 /access-denied`,
а `accessDeniedHandler` отдаёт свою страницу **со статусом 403** — шлюз
перехватывал его и слал `302` на неё же. Браузер крутился, пока не сдавался.

Отсюда правило при правке конфига: **`error_page` для перехватываемого кода
должен указывать на файл, а не на редирект.** Внутренний редирект на статику
терминален; редирект же возвращается клиенту, и если его цель отдаёт тот же
код — получается цикл.

Цели оставшихся редиректов (`/login`, `/menu`, `/services/<key>`) отдают
`200`, поэтому цепочка обрывается на первом шаге.
...
- Тема страницы — та же, что выбрана на портале: cookie `gh_theme`, дальше
  `localStorage`, дальше системная. Читают её два независимых пути: SSI
  подставляет класс в `<html>` (тема стоит до первой отрисовки, работает без
  JS), а инлайновый скрипт делает то же самое сам — на случай, когда SSI не
  отработал, например конфиг в контейнере устарел.
