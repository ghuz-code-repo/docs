...
`proxy_intercept_errors` включён **только в сгенерированных конфигах конечных
сервисов** — в блоке `location <prefix>/` (шаблон в
`auth-service/routes/nginx_config.go`). На уровне server-блока он выключен
явно, в `errors.inc`.

Так сделано не из осторожности, а по необходимости. Страницы самого портала
отдают `401` и `403` как обычный контент:

| Что | Код | Файл |
|---|---|---|
| форма логина с «неверный пароль» | `401` | `routes/auth_handlers.go` |
| `error.html` админки | `403` | `routes/middleware.go` |
| `access-denied.html` | `403` | `routes/routes.go` |

С перехватом на весь server-блок эти ответы попадали под `error_page 401/403`
и превращались в редиректы. Для `/access-denied` это давало **цикл**: страница
отдаёт `403`, шлюз шлёт `302` на неё же, браузер крутится до отказа. У формы
логина терялось сообщение об ошибке.

Правило простое: **ошибку показывает тот, у кого есть контекст.** У портала он
есть — он рисует свои страницы сам. У конечного сервиса его нет, там перехват
и включается.

Не перехватываются и внутри сервиса:

| Где | Почему |
|---|---|
| `<prefix>/static/` | ассетам нужен код ответа, а не страница на несколько килобайт |
...
умеет, поэтому на странице остаётся то, что видно снаружи: код,
`$upstream_status`, `$upstream_addr`, адрес запроса, время и `$request_id`.
Сервис, которому есть что сказать, кладёт текст в заголовки ответа
`X-Error-Code` и `X-Error-Detail` — шлюз читает их как `$upstream_http_*`
и показывает в блоке подробностей.

Обратная сторона: **JSON-ответы сервиса с кодом 4xx и 5xx под его префиксом до
клиента не доходят** — `fetch()` получит HTML-страницу. Сервисам с собственным
API нужно либо проверять `content-type`, либо отдавать прикладные ошибки кодом
`200`. Ветка «JSON вместо страницы» заготовлена в `errors.inc` и включается
вместе с `map $http_accept $gw_wants_json` в `nginx.conf`.

### Сколько редиректов видит браузер

Не больше одного на любую ошибку:

| Ошибка | Что делает браузер |
|---|---|
| `401` от `auth_request` | один `302` на `/login?redirect=…`, дальше страница логина `200` |
| `403` от `auth_request` | один `302` на `/access-denied?service=…`, дальше страница `403` с содержимым |
| `403` на `/logs` | один `302` на `/menu` |
| `403` на `/services/<key>/logs` | один `302` на `/services/<key>` |
| всё остальное | ноль редиректов: страница отдаётся прямо на исходном URL с исходным кодом |

Цепочка обрывается на первом шаге именно потому, что цели редиректов
(`/login`, `/access-denied`, `/menu`, `/services/<key>`) проксируются в
auth-service **без перехвата**: их собственные коды ответа нового `error_page`
уже не запускают.

Отсюда правило при правке конфига: включать `proxy_intercept_errors` на
маршруте, который служит целью редиректа, нельзя — это немедленно даёт цикл.
