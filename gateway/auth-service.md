...
Шаблон, кроме маршрутизации, обслуживает страницы ошибок:

- `set $gw_service_key <key>;` — имя раздела, которое попадёт на страницу,
  если сервис вернёт `5xx` или не ответит. Строка стоит **до**
  `rewrite ... break`: флаг `break` обрывает фазу rewrite, и `set` после него
  не выполнился бы.
- `proxy_intercept_errors off;` в блоках `<prefix>/static/` и
  `<prefix><health_path>` — ассетам и health-опросу нужен код ответа, а не
  HTML-страница. Остальные ответы сервиса перехватываются: `on` объявлен на
  уровне server-блока в `errors.inc`.

Правка любой из этих строк меняет поведение всех сервисов сразу — шаблон
входит в четыре поверхности совместимости (AGENTS.md §9).
