...
`proxy_intercept_errors on` включён на весь server-блок. Ответ сервиса с кодом
`4xx` или `5xx` пользователю **не доходит**: nginx выбрасывает тело и рисует
свою страницу с этим же кодом. Исключения — `/{key}/static/` и
`/{key}/health`, там перехват выключен.
