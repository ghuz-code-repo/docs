...
### 8.1. Ошибки сервиса заменяются страницей шлюза

`proxy_intercept_errors on` объявлен на весь server-блок шлюза. Ответ сервиса
с кодом `4xx` или `5xx` пользователю **не доходит**: nginx выбрасывает тело и
рисует свою страницу с этим же кодом. Исключения в вашем конфиге —
`/{key}/static/` и `/{key}/health`, там перехвата нет.

Это касается и страниц самого портала: единая стилизованная страница на любую
ошибку — осознанное решение, а не побочный эффект.

Два следствия, которые надо учесть в коде сервиса:

* **Своя HTML-страница ошибки не понадобится** — её никто не увидит.
* **JSON-ответы с кодом ошибки под своим префиксом до `fetch()` не дойдут.**
  Клиент получит HTML-страницу с тем же статусом. Если у сервиса есть свой
  API для собственного фронтенда, либо проверяйте `content-type` перед
  `response.json()`, либо отдавайте прикладные ошибки кодом `200` с телом
  `{"success": false, ...}`.

Чтобы пользователь увидел причину, а администратор нашёл её в логах, сервис
кладёт описание в **заголовки ответа** — тело недоступно шлюзу по устройству:

```http
HTTP/1.1 500 Internal Server Error
X-Error-Code: report_build_failed
X-Error-Detail: Не удалось собрать отчёт за 2026 год: нет данных за декабрь
```

Оба заголовка необязательны. Что попало в них, показывается в блоке
«Технические подробности» на странице ошибки — вместе с именем сервиса,
адресом узла и `$request_id`. Значения экранируются шлюзом.

Секретов, стектрейсов и SQL в `X-Error-Detail` быть не должно: заголовок
видит пользователь.
...
| `fetch()` получает HTML вместо JSON на ошибке | шлюз перехватывает `4xx`/`5xx` и подменяет тело страницей | раздел 8.1 |
