...
`proxy_intercept_errors on` стоит в блоке `location /{key}/` вашего
сгенерированного конфига. Ответ сервиса с кодом `4xx` или `5xx` пользователю
**не доходит**: nginx выбрасывает тело и рисует свою страницу с этим же кодом.
Исключения — `/{key}/static/` и `/{key}/health`, там перехвата нет.

На маршрутах самого портала перехват выключен: его страницы отдают `401` и
`403` как контент и рисуют ошибки сами.
